...
async function resolveAlbumArtwork(album, useAi, aiConfig) {
    ...
    if (useAi && aiConfig && aiConfig.provider && aiConfig.key) {
        try {
            ...
            const prompt = `You are an elite graphic designer specializing in physical music CD packaging design. Design a realistic, modern, minimalist CD spine as a raw SVG element for the album "${cleanName}" by "${album.artist}".\n\n` +
                           `Strict Design Constraints:\n` +
                           `1. Root Element: MUST be <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 280 3000" width="100%" height="100%"> (exact 1:10 aspect ratio representing a single jewel case spine).\n` +
                           `2. Background Aesthetics: Create an inspiring background using harmonious color fills, SVG linear/radial gradients, or clean abstract geometric shapes that reflect the musical vibe of the artist and genre.\n` +
                           `3. Vertical Typography: Include prominent vertical text using <text> tags with clean, high-contrast, professional styling (e.g., sans-serif or artistic font families, bold weights, and generous letter-spacing). Rotate the text (using transform="rotate(270, 140, 1500)" or transform="rotate(90, 140, 1500)") so it reads cleanly along the length of the spine. Clearly display the artist name and album title.\n` +
                           `4. Commercial Authenticity: At the bottom or top end of the spine (near y=180 or y=2820), include a tiny simulated record label logo symbol (simple vector circles/lines) and a simulated catalog serial number (e.g. "CD-78219" in small font) to evoke an authentic physical commercial release.\n` +
                           `5. Do NOT draw external plastic jewel case glare, reflections, or 3D borders (our frontend CSS acrylic glass engine automatically projects realistic polycarbonate jewel case lighting over top of your artwork strip).\n\n` +
